...
Please provide a JSON response with the following enhancements:
{
  ...
  "confidence": 0.8
    ...
    const response = await fetch('http://localhost:11434/api/generate', {
      ...
      body: JSON.stringify({
        model: getOllamaModel && getOllamaModel(),
        prompt: prompt,
        stream: false,
        format: 'json',
...
Respond with only a number between 0.0 and 1.0:`;
      try {
        const response = await fetch('http://localhost:11434/api/generate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ model: getOllamaModel && getOllamaModel(), prompt, stream: false, options: { temperature: 0.1, num_predict: 10 } })
        });
        if (response.ok) {
          const data = await response.json();
          const similarity = parseFloat(data.response.trim());
          if (!isNaN(similarity) && similarity >= 0 && similarity <= 1) {
            similarities.push({ name: folder.name, id: folder.id, confidence: similarity, description: folder.description });
          }
        }
      } catch (folderError) {
        ...
        similarities.push({ name: folder.name, id: folder.id, confidence: basicSimilarity, description: folder.description, fallback: true });
      }
    }

    ...
    <div className="container-narrow py-fib-34">
      <div className="text-center mb-fib-21">
        ...
        <div className="flex items-center justify-center gap-fib-8 mt-fib-8">
          ...
          <button className="text-xs text-system-gray-500 hover:text-system-gray-700 underline" onClick={() => { try { const keys = ['complete-summary','complete-next-steps']; keys.forEach(k => window.localStorage.setItem(`collapsible:${k}`, 'false')); window.dispatchEvent(new Event('storage')); } catch {} }}>
            Collapse all
          </button>
        </div>
      </div>

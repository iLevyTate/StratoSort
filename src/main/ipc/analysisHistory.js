...
function registerAnalysisHistoryIpc({ ipcMain, IPC_CHANNELS, logger, getServiceIntegration }) {
  ...
  ipcMain.handle(IPC_CHANNELS.ANALYSIS_HISTORY.GET, withErrorLogging(logger, async (event, options = {}) => {
    try {
      ...
      if (all || limit === 'all') {
        const full = (await getServiceIntegration()?.analysisHistory?.getRecentAnalysis(Number.MAX_SAFE_INTEGER)) || [];
        if (offset > 0) return full.slice(offset);
        return full;
      }
      const effLimit = typeof limit === 'number' && limit > 0 ? limit : 50;
      if (offset > 0) {
        const interim = (await getServiceIntegration()?.analysisHistory?.getRecentAnalysis(effLimit + offset)) || [];
        return interim.slice(offset, offset + effLimit);
      }
      return (await getServiceIntegration()?.analysisHistory?.getRecentAnalysis(effLimit)) || [];
    } catch (error) {

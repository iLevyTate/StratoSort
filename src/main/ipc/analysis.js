...
function registerAnalysisIpc({
  ...
  const analyzeDocumentHandler = async (event, filePath) => {
    try {
  ...
  ipcMain.handle(
    ...
    z && stringSchema
      ? withValidation(logger, stringSchema, analyzeDocumentHandler)
      : withErrorLogging(logger, analyzeDocumentHandler),
  );

  const analyzeImageHandler = async (event, filePath) => {
    try {
  ...
  ipcMain.handle(
    ...
    z && stringSchema
      ? withValidation(logger, stringSchema, analyzeImageHandler)
      : withErrorLogging(logger, analyzeImageHandler),
  );

  const ocrHandler = async (event, filePath) => {
    try {
  ...
  ipcMain.handle(
    ...
    z && stringSchema
      ? withValidation(logger, stringSchema, ocrHandler)
      : withErrorLogging(logger, ocrHandler),
  );
}

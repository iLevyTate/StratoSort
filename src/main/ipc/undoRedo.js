...
function registerUndoRedoIpc({ ipcMain, IPC_CHANNELS, logger, getServiceIntegration }) {
  // Undo
  ipcMain.handle(IPC_CHANNELS.UNDO_REDO.UNDO, withErrorLogging(logger, async () => {
    try {
  ...
  // Redo
  ipcMain.handle(IPC_CHANNELS.UNDO_REDO.REDO, withErrorLogging(logger, async () => {
    try {
  ...
  // History
  ipcMain.handle(IPC_CHANNELS.UNDO_REDO.GET_HISTORY, withErrorLogging(logger, async (event, limit = 50) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.UNDO_REDO.CLEAR_HISTORY, withErrorLogging(logger, async () => {
    try {
  ...
  // Status
  ipcMain.handle(IPC_CHANNELS.UNDO_REDO.CAN_UNDO, withErrorLogging(logger, async () => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.UNDO_REDO.CAN_REDO, withErrorLogging(logger, async () => {
    try {

...
function ReadyFileItem({
  ...
  return (
    <div className={`border rounded-lg p-13 transition-all duration-200 ${isSelected ? 'border-stratosort-blue bg-stratosort-blue/5' : 'border-system-gray-200'} overflow-x-auto`}>
      <div className="flex items-start gap-13">
        <input type="checkbox" checked={isSelected} onChange={() => onToggleSelected(index)} className="form-checkbox mt-3" />
          ...
          {analysis ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <div>
                  <label className="block text-xs font-medium text-system-gray-700 mb-2">Suggested Name</label>
                  <Input type="text" value={suggestedName} onChange={(e) => onEdit(index, 'suggestedName', e.target.value)} className="text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-system-gray-700 mb-2">Category</label>
            ...
            <div className="text-sm text-system-red-600 mt-3">Analysis failed - will be skipped</div>
          )}
        </div>

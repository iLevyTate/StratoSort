  ...
  ipcMain.handle(IPC_CHANNELS.EMBEDDINGS.REBUILD_FILES, async () => {
    try {
      ...
      if (!historyService?.getRecentAnalysis) {
        return { success: false, error: 'Analysis history service unavailable' };
      }
      ...
      const smartFolders = (typeof getCustomFolders === 'function' ? getCustomFolders() : []).filter(f => f && f.name);
      ...
      for (const entry of allEntries) {
        try {
          ...
          const summary = [
            ...
            entry.analysis?.extractedText ? String(entry.analysis.extractedText).slice(0, 2000) : ''
          ].filter(Boolean).join('\n');

...
class FolderMatchingService {
  ...
  async embedText(text) {
    ...
    const model = getOllamaEmbeddingModel() || 'nomic-embed-text';
    const { embedding } = await client.embeddings({ model, prompt: text.slice(0, 8000) });
  ...
  async upsertFolderEmbedding(folder) {
    ...
    if (existing && existing.name === folder.name && (existing.description || '') === (folder.description || '')) {
      return existing;
    }
    const text = `${folder.name}\n${folder.description || ''}`.trim();
    const { vector, model } = await this.embedText(text);
    const payload = { id: folder.id, name: folder.name, description: folder.description || '', vector, model, updatedAt: new Date().toISOString() };
  ...
  async upsertFileEmbedding(fileId, contentSummary, fileMeta = {}) {
    ...
    await this.embeddingStore.upsertFile({ id: fileId, vector, model, meta: fileMeta, updatedAt: new Date().toISOString() });
  }
